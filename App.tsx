...
import { CsvRow, ProcessingStatus, FileData, NamingConfig, NamingStrategy } from './types';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12 items-start">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6 h-full">
          <div 
            ...
            className={`p-6 bg-white rounded-3xl shadow-sm border-2 h-full transition-all flex flex-col ${
              pdfFile ? 'border-indigo-100 shadow-indigo-50/50' : isDraggingPdf ? 'border-indigo-400 bg-indigo-50' : 'border-gray-100'
            }`}
          >
              ...
              <div className="flex p-1 bg-gray-100 rounded-2xl mb-8 overflow-x-auto">
                {(['default', 'custom', 'csv', 'manual'] as NamingStrategy[]).map((strat) => (
                  <button
                    ...
                    className={`flex-1 min-w-[100px] py-2.5 px-4 rounded-xl text-sm font-black transition-all capitalize whitespace-nowrap ${
                      namingConfig.strategy === strat 
                  ...
                  <div className="space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
                    {!csvFile ? (
                      <label 
                        ...
                        className={`flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-3xl cursor-pointer hover:bg-slate-50 transition-all ${
                          isDraggingCsv ? 'border-indigo-400 bg-indigo-50' : 'border-slate-200'
                        }`}
                      >
        ...
        {!isProcessing && !status?.isComplete && (
          <div className="flex flex-col items-center gap-6">
            <button
              ...
              className={`px-16 py-6 rounded-3xl font-black text-xl text-white shadow-[0_20px_50px_rgba(79,70,229,0.3)] flex items-center gap-4 transition-all transform hover:scale-105 active:scale-95 group ${
                (!pdfFile || (namingConfig.strategy === 'csv' && !csvFile)) 
